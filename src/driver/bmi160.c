#include "ESP32EmbeddedCommonLib/driver/bmi160.h"
...
static esp_err_t ecl_bmi160_write(const ecl_bmi160_t *imu, uint8_t reg, uint8_t val) {
    uint8_t buf[2] = {reg, val};
    return i2c_master_transmit(imu->dev_handle, buf, sizeof(buf), -1);
}

/* Read one or more bytes from a BMI160 register over I2C. */
static esp_err_t ecl_bmi160_read(const ecl_bmi160_t *imu,
...
static esp_err_t ecl_bmi160_wait_cmd_ready(const ecl_bmi160_t *imu) {
    uint32_t elapsed_ms = 0;
    while (elapsed_ms < BMI160_CMD_POLL_TIMEOUT_MS) {
        uint8_t status = 0;
        esp_err_t err = ecl_bmi160_read(imu, BMI160_REG_STATUS, &status, 1);
        if (err != ESP_OK) {
            return err;
        }
...
static float ecl_bmi160_accel_scale_from_range(ecl_bmi160_accel_range_t range) {
    switch (range) {
...
static float ecl_bmi160_gyro_scale_from_range(ecl_bmi160_gyro_range_t range) {
    switch (range) {
...
esp_err_t ecl_bmi160_init(
    ...
    err = ecl_bmi160_write(imu, BMI160_REG_CMD, BMI160_CMD_SOFTRESET);
    if (err != ESP_OK) {
    ...
    /* Verify chip ID */
    uint8_t chip_id = 0;
    err = ecl_bmi160_read(imu, BMI160_REG_CHIP_ID, &chip_id, 1);
    if (err != ESP_OK) {
    ...
    err = ecl_bmi160_wait_cmd_ready(imu);
    if (err != ESP_OK) {
        i2c_master_bus_rm_device(imu->dev_handle);
        return err;
    }
    err = ecl_bmi160_write(imu, BMI160_REG_CMD, BMI160_CMD_ACC_NORMAL);
    if (err != ESP_OK) {
        i2c_master_bus_rm_device(imu->dev_handle);
        return err;
    }
    ...
    err = ecl_bmi160_write(imu, BMI160_REG_CMD, BMI160_CMD_GYR_NORMAL);
    if (err != ESP_OK) {
        i2c_master_bus_rm_device(imu->dev_handle);
        return err;
    }
    ...
    err = ecl_bmi160_read(imu, BMI160_REG_PMU_STATUS, &pmu_status, 1);
    if (err != ESP_OK) {
        i2c_master_bus_rm_device(imu->dev_handle);
        return err;
    }
    ...
    err = ecl_bmi160_write(imu, BMI160_REG_ACC_RANGE, (uint8_t)config->accel_range);
    if (err != ESP_OK) {
        i2c_master_bus_rm_device(imu->dev_handle);
        return err;
    }

    err = ecl_bmi160_write(imu, BMI160_REG_GYR_RANGE, (uint8_t)config->gyro_range);
    if (err != ESP_OK) {
        i2c_master_bus_rm_device(imu->dev_handle);
        return err;
    }

    imu->accel_scale = ecl_bmi160_accel_scale_from_range(config->accel_range);
    imu->gyro_scale  = ecl_bmi160_gyro_scale_from_range(config->gyro_range);
...
esp_err_t ecl_bmi160_read_accel(
    ...
    uint8_t buf[6];
    esp_err_t err = ecl_bmi160_read(imu, BMI160_REG_ACCEL_DATA, buf, sizeof(buf));
    if (err != ESP_OK) {
        return err;
    }
...
esp_err_t ecl_bmi160_read_gyro(
    ...
    uint8_t buf[6];
    esp_err_t err = ecl_bmi160_read(imu, BMI160_REG_GYRO_DATA, buf, sizeof(buf));
    if (err != ESP_OK) {
        return err;
    }
...
static esp_err_t bmi160_imu_read_accel(void *ctx, ecl_sensor_imu_vec3_t *accel_g)
{
    return ecl_bmi160_read_accel((const ecl_bmi160_t *)ctx, accel_g);
}

static esp_err_t bmi160_imu_read_gyro(void *ctx, ecl_sensor_imu_vec3_t *gyro_dps)
{
    return ecl_bmi160_read_gyro((const ecl_bmi160_t *)ctx, gyro_dps);
}

static esp_err_t bmi160_imu_deinit(void *ctx)
{
    return ecl_bmi160_deinit((ecl_bmi160_t *)ctx);
}

esp_err_t ecl_bmi160_bind_imu(ecl_bmi160_t *bmi160, ecl_sensor_imu_t *out)
{
    if (bmi160 == NULL || out == NULL) {
        return ESP_ERR_INVALID_ARG;
    }

    out->read_accel = bmi160_imu_read_accel;
    out->read_gyro  = bmi160_imu_read_gyro;
    out->deinit     = bmi160_imu_deinit;
    out->ctx        = bmi160;
