...
void ecl_algo_filter_complementary_init(
...
float ecl_algo_filter_complementary_update(
    ecl_complementary_filter_t *f,
    float gyro_dps,
...
void ecl_algo_filter_moving_avg_init(
    ecl_moving_avg_t *f,
    uint32_t window)
{
    if (f == NULL) return;
...
float ecl_algo_filter_moving_avg_update(
    ecl_moving_avg_t *f,
    float value)
{
...
void ecl_algo_filter_moving_avg_reset(ecl_moving_avg_t *f)
{
    if (f == NULL) return;
    memset(f->buf, 0, sizeof(f->buf));
    f->head  = 0;
...
float ecl_algo_filter_lpf_update(ecl_lpf_t *f, float input)
{
