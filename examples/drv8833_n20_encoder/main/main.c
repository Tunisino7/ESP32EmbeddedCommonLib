...
static void log_motors(void)
{
    ...
    ecl_motor_dc_encoder_get_rpm(&s_motor_a, &rpm_a);
    ecl_motor_dc_encoder_get_rpm(&s_motor_b, &rpm_b);
    ecl_motor_dc_encoder_get_pulses(&s_motor_a, &pulses_a);
    ecl_motor_dc_encoder_get_pulses(&s_motor_b, &pulses_b);
...
static void motor_task(void *arg)
{
    ...
    ESP_ERROR_CHECK(ecl_motor_dc_encoder_set_speed(&s_motor_a,  60));
    ESP_ERROR_CHECK(ecl_motor_dc_encoder_set_speed(&s_motor_b,  60));
    ...
    ESP_ERROR_CHECK(ecl_motor_dc_encoder_set_speed(&s_motor_a, -40));
    ESP_ERROR_CHECK(ecl_motor_dc_encoder_set_speed(&s_motor_b, -40));
    ...
    /* ── 3. Stop ──────────────────────────────────────────────────────────── */
    ESP_LOGI(TAG, "Stop");
    ESP_ERROR_CHECK(ecl_motor_dc_encoder_stop(&s_motor_a));
    ESP_ERROR_CHECK(ecl_motor_dc_encoder_stop(&s_motor_b));
    ...
    bool fault = false;
    if (ecl_driver_drv8833_is_fault(&s_bridge, &fault) == ESP_OK && fault) {
        ESP_LOGE(TAG, "DRV8833 nFAULT asserted — overcurrent or overtemperature!");
    }

    /* Reset counters and loop back */
    ESP_ERROR_CHECK(ecl_motor_dc_encoder_reset_count(&s_motor_a));
    ESP_ERROR_CHECK(ecl_motor_dc_encoder_reset_count(&s_motor_b));
...
void app_main(void)
{
    ...
    ecl_drv8833_config_t bridge_cfg =
        ecl_driver_drv8833_default_config(PIN_AIN1, PIN_AIN2,
                                             PIN_BIN1, PIN_BIN2);
     ...
     * ecl_driver_drv8833_set_speed() and
     * ecl_motor_dc_encoder_set_speed() on the same channel at the
    ...
    ESP_ERROR_CHECK(ecl_motor_dc_encoder_init(&s_motor_a, &motor_a_cfg));
    ...
    ESP_ERROR_CHECK(ecl_motor_dc_encoder_init(&s_motor_b, &motor_b_cfg));
